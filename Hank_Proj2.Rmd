...
We set $\epsilon=1E-9$, choose inital values $\pmb{x}^{(0)}=(x^{(0)}, y^{(0)})=(1, 1)$ and begin to iterate.
```{r}
...
For each value of $k$, $\lambda_1$, and $\lambda_2$ we want to asses the accuracy of the model somehow. We will use as our critereon the cross-validation error 
...
So for the sake of speed we will create a vector of (decreasing) `lambda` values to pass to the `glmnet()` function with each call and repeat this for several values of `alpha`, supplied in seperate calls. We will do this for each of our k (10) folds, computing the validation errors, $CV_k(\alpha, \lambda)$, as we go.
...
The object `results` is now a list with k elements and each element is a `data.frame` with fields `k`, `alpha`, `lambda`, and `MSE`. We combine these homogenous list elements into one `data.frame` and find the average MSE for each set of `alpha` and `lambda` values (averaging the k MSE values calculated for each pair).
```{r, cache=TRUE}
