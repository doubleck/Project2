...
SVDD <- function(x, k){
  ...
  fun_H <- (2)*H
  fun_c <- (-1)*diag(H)
  fun_A <- rep(1, nrow(my_matrix))
  fun_b <- 1
  fun_l <- rep(0, nrow(my_matrix))
  fun_u <- rep(1, nrow(my_matrix))
  fun_r <- 0
...
c. Write an _R_ function for detecting potential outliers for a new set of observations, along with the upper threshold.
```{r}
...
(d) Write an _R_ function for detecting potential outliers for a new set of observations, along with the upper threshold.
